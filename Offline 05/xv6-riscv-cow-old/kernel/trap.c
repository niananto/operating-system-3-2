...
usertrap(void)
{
  ...
  if(r_scause() == 8){ // 8 means page fault
    // system call
    ...
    // 0 means trap generated by CPU [system call, page fault]
    // so we are not interested in any other trap
    ...
    if((flags & PTE_V) && (flags & PTE_U) && (flags & PTE_RSW)){
      ...
      uvmunmap(p->pagetable, start_va, 1, 0); // 1 or PGSIZE?
      sub_rc(pa);
      if (mappages(p->pagetable, start_va, 1, (uint64)mem, flags) != 0) {
